...
| 04 | [`04-auth-protocols-gateway-audit.md`](04-auth-protocols-gateway-audit.md) | **Bảo mật + kiến trúc** của auth (cookie session/CSRF/Bearer/Basic/OIDC/JWT), 4 giao thức giao tiếp (REST/gRPC/GraphQL/RabbitMQ), `graphql-gateway` — chỉ core `metap`, `metap-lowcode` để lại lần sau (2026-09-03) | **17 finding, chưa fix cái nào** — Phần A bảo mật (1 HIGH, 4 MEDIUM, 5 LOW), Phần B kiến trúc (1 HIGH, 3 MEDIUM, 3 LOW). Xem 2 bảng bên dưới |

## Chi tiết audit 04 — Phần A, bảo mật (chưa xử lý)

| # | Mức độ | Vấn đề | Trạng thái |
|---|---|---|---|
| 1 | **HIGH** | SSRF có phản hồi qua `webhook` target của cron — tenant admin đọc được nội bộ mạng + cloud metadata | Chưa fix |
...
## Chi tiết audit 04 — Phần B, kiến trúc (chưa xử lý)
...
| B4 | MEDIUM | Cookie/CSRF session (ship 2026-09-03) có **0 test** — `metap_session`/`x-csrf-token` không xuất hiện trong `crates/*/tests/` | Chưa fix — 4 ca test tối thiểu đã liệt trong audit |
| B5 | LOW | `attach_trace_context` không có caller nào → trace liền qua gRPC nhưng đứt qua mọi hop REST | Chưa fix — sửa rẻ, 3 call site |
