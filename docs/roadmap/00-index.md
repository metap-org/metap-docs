...
| [90. Bỏ REST entity CRUD ở `metap` core, GraphQL-only](90-remove-rest-entity-crud.md) | `metap` PR #31 — xoá hẳn `routes::records` (`/api/:entity*`), giữ REST cho `/auth/*` + webhook-receiving + vài route "structural" (OAuth2 AS, `.well-known`, health/metrics, attachments). `cron-scheduler`'s `workflow_transition`/`bulk_query_action` migrated sang gRPC. Gap có sẵn: `AUTH_CONTEXT_ENTITY` cache-invalidation chỉ có ở REST handler cũ, giờ unconditional cho GraphQL/gRPC — chưa fix |
| [91. Apollo Federation v2 capability cho `metap-graphql` (opt-in)](91-graphql-federation-capability.md) | `build_schema_parts_with_federation`/`build_schema_with_federation` (additive, hàm cũ không đổi) — `.key("id")` + `entity_resolver` tái dùng đúng `RecordBackend::get`; chưa service nào bật, `metap-graphql-gateway` không đụng. Verify sống lộ ra union `_Entity` không có per-item-null — sửa thành trả lỗi GraphQL thật thay vì null giả |
